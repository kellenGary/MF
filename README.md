# MF — Music + Social (mobile-first)

MF is a cross-platform music-focused social app: an Expo (React Native + TypeScript) frontend paired with a .NET Web API backend. The app provides feeds, user profiles, playlists, playback controls, Spotify sync hooks, and lightweight social features (posts, follows, listening history).

**Repository layout**
- **app/**: Expo app (file-based routing, TypeScript + React Native UI, components under `components/` and `ui/`).
- **api/MFAPI/**: ASP.NET Web API project (controllers, models, EF Core DbContext).
...
By default the API reads settings from `api/MFAPI/appsettings.Development.json` / `appsettings.json`. Update the connection string or other settings there as needed.
...
- The backend has controllers under `api/MFAPI/Controllers/` and EF models under `api/MFAPI/Models/`.
...
- Add a backend endpoint: edit/ add a controller in `api/MFAPI/Controllers/` and update `AppDbContext` if you add models.
...
- API `.env` (see `api/MFAPI/.env.example`):
...
2. (Optional) Copy `api/MFAPI/.env.example` to `api/MFAPI/.env` and update DB/secret values.
